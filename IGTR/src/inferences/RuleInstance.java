...
import util.digraph.graph.GraphTUtil;
...
public class RuleInstance {
	...
	public RuleInstance(String strRruleName, ArrayList<GParameter> strRuleParameters, GraphT insLHS, GraphT insRHS) {
		this(strRruleName, strRuleParameters, insLHS, insRHS, false, 0);
	}

	public RuleInstance(String strRruleName, ArrayList<GParameter> strRuleParameters, GraphT insLHS, GraphT insRHS,
			boolean IsNac, int INacReferenceID) {
	...
	protected boolean save() {
		...
		if (this.ruleInsID < 1
				|| !DBRecord.endCurrentObservation(this.ruleInsID, this.ruleName, this.strGetRuleParameters(), "")) {
		...
		// save meta
		if (!DBRecord.initialSavingBasicRule(this.ruleInsID, this.ruleName, !this.isNAC, 0, // iTotalExecutedObjects
				0, // iScopeObjectsCount
				this.iObjectsCount, 0, // internalStateStep
				this.hasEffect, this.iNacReferenceID, // parentRuleId
		...
		if (this.isNAC) {
			System.out.println("NAC instance saved: id= " + this.ruleInsID + " (rule reference id:"
					+ this.iNacReferenceID + ")");
		} else {
	...
	private void adjustGraphElements() {
		...
		for (int iNode = 0; iNode < this.gLHS.gNodes.size(); iNode++) {
			...
			if (postNode == null) {
				...
				// this just to style deleted object
				preNode.isInitialized = true;
			} else {

				// set (this) node
				if (!setThisObject) {
					if (GraphTUtil.isNormalNode(preNode)){
						// only normal nodes may serve as this-object
						preNode.isThis = true;
	...
	private void updateMinimalNodeFromEdgeMinimal(int iPreGraphID, int iPostGraphID) {
		...
		String strSQLStatement = "update TblNode " + " set isMinimal=true " + " where (Graph_IDREFF=" + iPreGraphID
				+ " or Graph_IDREFF=" + iPostGraphID + ")" + " and isMinimal=false " + " and nodeID IN ("
				+ "	select sourceID as nodeID from TblEdge " + "	where (Graph_IDREFF=" + iPreGraphID
				+ " or Graph_IDREFF=" + iPostGraphID + ")" + "	and isMinimal=true " + "	union"
				+ "	select targetID as nodeID from TblEdge " + "	where (Graph_IDREFF=" + iPreGraphID
				+ " or Graph_IDREFF=" + iPostGraphID + ")" + "	and isMinimal=true " + ");" + ""
				+ "DROP TEMPORARY TABLE IF EXISTS tblTemp3;" + "CREATE TEMPORARY TABLE IF NOT EXISTS tblTemp3 AS ("
				+ "	select sourceID as nodeID from TblEdge " + "	where (Graph_IDREFF=" + iPreGraphID
				+ " or Graph_IDREFF=" + iPostGraphID + ")" + "	and isMinimal=false " + "	and targetID in ("
				+ "		select nodeID as targetID from TblNode" + "		where (Graph_IDREFF=" + iPreGraphID
				+ " or Graph_IDREFF=" + iPostGraphID + ")" + "		and isMinimal=true" + "		and isCollection=true));" + ""
				+ "update TblNode " + " set isMinimal=true " + " where (Graph_IDREFF=" + iPreGraphID
				+ " or Graph_IDREFF=" + iPostGraphID + ")" + " and isMinimal=false " + " and isCollection=false "
				+ " and nodeID IN (select nodeID from tblTemp3);" + "" + "update TblEdge " + " set isMinimal=true "
				+ " where (Graph_IDREFF=" + iPreGraphID + " or Graph_IDREFF=" + iPostGraphID + ")"
				+ " and isMinimal=false " + " and sourceID in (" + "		select nodeID as targetID from TblNode"
				+ "		where (Graph_IDREFF=" + iPreGraphID + " or Graph_IDREFF=" + iPostGraphID + ")"
				+ "		and isMinimal=true)" + " and targetID in (" + "		select nodeID as targetID from TblNode"
				+ "		where (Graph_IDREFF=" + iPreGraphID + " or Graph_IDREFF=" + iPostGraphID + ")"
				+ "		and isMinimal=true" + "		and isCollection=true);" + "";
	...
	private String strGetRuleParameters() {
		...
		for (int iPar = 0; iPar < this.ruleParameters.size(); iPar++) {
			...
			strParameters += getPar.ParType + " " + getPar.ParName + " = " + getPar.ParValue;
