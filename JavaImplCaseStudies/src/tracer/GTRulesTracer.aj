...
public aspect GTRulesTracer {
	...
	private static Set<String> methodsToBeObserved = getMethodsToBeObserved();
			
	private static Set<String> getMethodsToBeObserved(){
		
		Set<String> collectionMethods = new HashSet<String>();
		
		collectionMethods.add("registerClient");
		collectionMethods.add("makeReservation");
		collectionMethods.add("cancelReservation");
		collectionMethods.add("cancelClientReservation");
		collectionMethods.add("pickupCar");
		collectionMethods.add("dropoffCar");
	...
	before() : stateTriggers() {
		...
		if (thisJoinPoint.getKind().equals(JoinPoint.METHOD_CALL)){			
			...
			String strCalledMethodName = thisJoinPoint.getSignature().getName();
